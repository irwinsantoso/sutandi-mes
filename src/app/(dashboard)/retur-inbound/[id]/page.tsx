...
import { ReturInboundDetailClient } from "./_components/retur-inbound-detail-client"

interface PageProps {
  params: Promise<{ id: string }>
}

export default async function ReturInboundDetailPage({ params }: PageProps) {
  const { id } = await params

  const t = await prisma.inboundTransaction.findUnique({
    where: { id },
    include: {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Retur Inbound ${t.transactionNumber}`}
        description="Pengembalian barang dari customer"
      >
        <Button variant="outline" render={<Link href="/retur-inbound" />}>
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              ...
              <p className="font-medium">{format(t.receivingDate, "dd MMM yyyy")}</p>
            </div>
            <div>
            ...
            {t.referenceNumber && (
              <div>
                ...
                <p className="font-medium">{t.referenceNumber}</p>
              </div>
            )}
            {t.notes && (
      ...
      {t.status === "DRAFT" && <ReturInboundDetailClient id={t.id} />}
    </div>
  )
}
