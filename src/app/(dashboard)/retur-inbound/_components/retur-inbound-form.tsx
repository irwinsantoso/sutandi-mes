...
import { createReturInbound } from "../_actions/retur-inbound-actions"

type UomInfo = { id: string; code: string; name: string }

interface ReturInboundFormProps {
  items: Array<{
    id: string
    code: string
    name: string
...
function getAvailableUoms(
  itemId: string,
  items: ReturInboundFormProps["items"]
): UomInfo[] {
  ...
  for (const conv of item.uomConversions) {
    if (!uomMap.has(conv.fromUom.id)) uomMap.set(conv.fromUom.id, conv.fromUom)
    if (!uomMap.has(conv.toUom.id)) uomMap.set(conv.toUom.id, conv.toUom)
  }
...
export function ReturInboundForm({ items, locations, projectNames }: ReturInboundFormProps) {
  ...
  function handleSubmit() {
    ...
    const validItems = lineItems.filter(
      (li) => li.itemId && li.quantity && li.uomId && li.locationId
    )
    ...
    startTransition(async () => {
      const result = await createReturInbound({
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Retur Details</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                id="projectName"
                list="retur-inbound-project-names"
                placeholder="Pilih atau ketik nama proyek"
                value={projectName}
                onChange={(e) => setProjectName(e.target.value)}
              />
              <datalist id="retur-inbound-project-names">
                {projectNames.map((p) => (
            ...
            <div className="space-y-2">
              <Label htmlFor="customerName">Customer (Pengembali)</Label>
              <Input
                id="customerName"
                placeholder="Nama customer yang mengembalikan"
                value={customerName}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="referenceNumber"
                placeholder="e.g. DO/Invoice asal (optional)"
                value={referenceNumber}
                onChange={(e) => setReferenceNumber(e.target.value)}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="receivingDate">Tanggal Penerimaan Retur</Label>
              <Input
                id="receivingDate"
                type="date"
                value={receivingDate}
            ...
            <div className="space-y-2 sm:col-span-2">
              ...
              <Textarea
                id="notes"
                placeholder="Catatan retur (alasan, kondisi barang, dll)"
                value={notes}
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="Batch #"
                    value={li.batchLot}
                    onChange={(e) => updateLineItem(li.key, "batchLot", e.target.value)}
                  />
                </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="outline" type="button" render={<Link href="/retur-inbound" />}>
          Cancel
        </Button>
      </div>
    </div>
